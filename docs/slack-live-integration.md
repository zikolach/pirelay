...
The harness writes per-instance config files under a temporary directory, points each Pi process at a distinct `PI_RELAY_CONFIG`/`PI_RELAY_STATE_DIR`, and passes the relevant Slack token/signing-secret/app-level token values via environment variables. Temporary state is deleted during teardown so repeated runs do not reuse stale local bindings. The current live Slack runtime is a receive-confirmation stub: it starts Socket Mode, falls back to bounded channel-history polling for the test channel, and posts `PiRelay Slack stub received...` so the suite can verify Pi-side receipt before full Slack prompt routing is implemented.
