...
6. Run `/relay setup discord` with `discord.clientId`/`PI_RELAY_DISCORD_CLIENT_ID` and verify the guidance includes a Discord invite URL, Message Content Intent guidance, DM-first guidance, and allow-list recommendations.
...
9. Run `/relay connect discord docs`, then DM the bot `/start <code>` before expiry. If the bot cannot be DM'd, check server member DM privacy settings and that the bot was invited with the `bot` scope.
