...
12. From local Pi, run `/relay send-file telegram README.md`, `/relay send-file discord README.md`, `/relay send-file slack README.md`, and `/relay send-file all README.md` against paired chats; verify safe delivery summaries and that paused/missing bindings are skipped without raw ids in normal UX.
13. Try local `/relay send-file slack ../secret.md`, absolute paths, hidden files, symlink escapes, oversized files, and unsupported binaries; verify no messenger API upload is attempted.
