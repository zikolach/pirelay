...
- `usedMessageLifecycleFallback: true` means the final `agent_end` payload omitted assistant text, but PiRelay safely completed the turn using non-empty assistant text already observed during message lifecycle events.
