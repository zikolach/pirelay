...
Remote skill invocation is opt-in via `skills.enabled`. When enabled, `/skills` lists allowed local Pi skills from the live session command registry, and `/skill <name> [input]` asks Pi to use that skill. If input is omitted, the next message from the same authorized requester is used as skill input; `/skill cancel` cancels the pending input.
