...
Discord and Slack foundations are opt-in. Discord now includes a live DM-first bot runtime when `discord.enabled` and `discord.botToken` are configured; run `/relay setup discord` for the interactive setup wizard with credential, intent, invite/QR, and DM troubleshooting guidance. In headless/no-UI contexts, `/relay setup <messenger>` falls back to secret-safe plain text guidance. Guild/channel control requires explicit authorization config.

The interactive setup wizard is secret-safe: it shows one tab at a time for diagnostics, env snippet, config snippet, links, or troubleshooting. Slack setup also includes an App manifest tab. Press `c` to copy the messenger env snippet to the system clipboard, press `m` in Slack setup to copy the Slack app manifest, or fall back to the Pi editor when no clipboard command is available. Press `w` to write/update canonical PiRelay config from currently defined env vars; this stores env var names such as `tokenEnv`, `signingSecretEnv`, and `appTokenEnv`, not resolved bot tokens, signing secrets, OAuth secrets, or peer secrets.
...
- Slack: create a Slack app, install it to your workspace, set the Bot User OAuth Token as `PI_RELAY_SLACK_BOT_TOKEN` or `slack.botToken`, set the Signing Secret as `PI_RELAY_SLACK_SIGNING_SECRET` or `slack.signingSecret`, and for Socket Mode set `PI_RELAY_SLACK_APP_TOKEN`/`slack.appTokenEnv`. `/relay setup slack` can copy a ready-to-paste Slack app manifest with App Home messages, Socket Mode, DM events, and required scopes. Set `PI_RELAY_SLACK_APP_ID` or `slack.appId` from Basic Information → App Credentials → App ID for App Home QR links. `/relay connect slack` shows a short `pirelay pair 123-456` PIN command and `c` copies it; either open the App Home DM from the QR/link and paste the command there, or invite the app to a channel and paste the command in that channel after enabling `slack.allowChannelMessages`. If Slack says “Sending messages to this app has been turned off,” enable App Home → Messages Tab → Allow users to send messages to your app, add the `message.im` event, `im:history`/`im:read` scopes, and `reactions:write` for thinking indicators, reinstall the app, then retry (<https://api.slack.com/apps>).
