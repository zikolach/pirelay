...
export class InProcessTunnelRuntime implements TunnelRuntime {
  ...
  async registerRoute(route: SessionRoute): Promise<void> {
    ...
    if (previousRoute?.binding?.chatId !== route.binding?.chatId && previousRoute?.binding) {
      ...
      this.clearProgressState(previousRoute);
    }
  ...
  private async flushProgress(sessionKey: string, chatId: number, userId: number | undefined, key: string): Promise<void> {
    ...
    const editableApi = this.api as TelegramApiClient & {
    ...
    await deliverLiveProgress(
      ...
      messageText,
      {
        updateLiveProgress: state.liveMessageRef && editableApi.editPlainText
          ? async (ref, nextText) => {
            const messageId = Number(ref);
            if (!Number.isSafeInteger(messageId)) {
              throw new Error("Telegram live message id is not numeric.");
            }
            await editableApi.editPlainText(chatId, messageId, nextText);
          }
          : undefined,
        sendLiveProgress: editableApi.sendEditablePlainText
          ? async (nextText) => {
            const messageId = await editableApi.sendEditablePlainText(chatId, nextText);
            return Number.isSafeInteger(messageId) ? String(messageId) : undefined;
          }
          : undefined,
        sendProgressSnapshot: (nextText) => this.api.sendPlainText(chatId, nextText),
      },
    );
  }
    ...
    if (action.kind === "dashboard" && action.sessionRef !== "current") {
      ...
      if (!route) {
        if (action.kind === "dashboard" && action.action === "forget") {
          const revoked = await this.store.revokeBinding(entry.sessionKey);
          ...
          if (!revoked) {
            await this.api.answerCallbackQuery(callback.callbackQueryId, "No matching offline session.");
            await this.api.sendPlainText(callback.chat.id, "No matching offline session found. Use /sessions for the latest list.");
            return undefined;
          }
    ...
    switch (action.kind) {
      case "answer-option": {
        ...
        if (!await this.sendPromptSafely(
          ...
          {
            ...(delivery.deliverAs ? { deliverAs: delivery.deliverAs } : {}),
            requester: this.telegramRequester(route, callback),
            onRollback: () => this.clearActivityIndicator(route),
          },
  ...
  private async handleTelegramDelegationMessage(message: TelegramInboundMessage, command: NonNullable<ReturnType<typeof parseDelegationCommand>>): Promise<boolean> {
    ...
    switch (decision.kind) {
      case "ignore":
        ...
        if (decision.message && !message.user.isBot) await this.api.sendPlainText(message.chat.id, decision.message);
        return true;
      case "reject":
        await this.api.sendPlainText(message.chat.id, decision.message);
        return true;
      case "render-task":
  ...
  private telegramChannelMessage(message: TelegramInboundMessage): ChannelInboundMessage {
    return {
      kind: "message",
      channel: "telegram",
      updateId: String(message.messageId),
      messageId: String(message.messageId),
      text: message.text,
      attachments: [],
      conversation: { channel: "telegram", id: String(message.chat.id), kind: isTelegramGroupConversation(message.chat.type) ? "group" : "private", title: message.chat.title },
      sender: {
        channel: "telegram",
        ...
        metadata: { isBot: message.user.isBot === true },
      },
    };
  }
  ...
  private async handleTelegramGroupSharedRoomCommand(message: TelegramInboundMessage, target: TelegramGroupCommandTarget | undefined): Promise<boolean> {
    const delegationCommand = target ? parseDelegationCommand(target.command, target.args) : undefined;
    if (!target || !["help", "sessions", "use", "to"].includes(target.command) && !delegationCommand) return false;
    ...
    switch (target.command) {
      ...
      case "to": {
        ...
        if (!await this.sendPromptSafely(targetRoute, message, resolution.prompt, {
          ...(delivery.deliverAs ? { deliverAs: delivery.deliverAs } : {}),
          requester: this.telegramRequester(targetRoute, message),
          onRollback: () => {
  ...
  private async sendPromptSafely(
    ...
    options?: { deliverAs?: "followUp" | "steer"; requester?: RelayFileDeliveryRequester; onRollback?: () => Promise<void> | void },
  ...
  private async deliverAuthorizedPrompt(
    ...
    if (!await this.sendPromptSafely(route, message, content, {
      ...
      requester: this.telegramRequester(route, message),
      onRollback: () => this.clearActivityIndicator(route),
    })) return;
  ...
  private async deliverPlainPrompt(route: SessionRoute, message: TelegramInboundMessage, text: string): Promise<void> {
    ...
    const activityStarted = await this.startActivityIndicator(route);
    if (!await this.sendPromptSafely(route, message, text, {
      ...(deliverAs ? { deliverAs } : {}),
      requester: this.telegramRequester(route, message),
      onRollback: () => this.clearActivityIndicator(route),
    })) return;
  ...
  private async resolveAmbiguity(route: SessionRoute, message: TelegramInboundMessage, pending: PendingAnswerAmbiguityState, resolution: "prompt" | "answer" | "cancel"): Promise<void> {
    ...
    this.clearAnswerStateForRoute(route);
    await this.startActivityIndicator(route);
    if (!await this.sendPromptSafely(route, message, result.injectionText, {
      ...(delivery.deliverAs ? { deliverAs: delivery.deliverAs } : {}),
      requester: this.telegramRequester(route, message),
      onRollback: () => this.clearActivityIndicator(route),
    })) return;
  ...
  private async handleAuthorizedMessage(route: SessionRoute, message: TelegramInboundMessage): Promise<void> {
    ...
    if (pendingCustom) {
      ...
      await this.startActivityIndicator(route);
      if (!await this.sendPromptSafely(route, message, buildFreeTextChoiceInjection(metadata, message.text), {
        ...(delivery.deliverAs ? { deliverAs: delivery.deliverAs } : {}),
        requester: this.telegramRequester(route, message),
        onRollback: () => this.clearActivityIndicator(route),
      })) return;
    ...
    if (metadata && (intent.kind === "bare-option" || (intent.kind === "explicit-answer" && intent.option))) {
      ...
      this.clearAnswerStateForRoute(route);
      await this.startActivityIndicator(route);
      if (!await this.sendPromptSafely(route, message, buildChoiceInjection(metadata, option), {
        ...(delivery.deliverAs ? { deliverAs: delivery.deliverAs } : {}),
        requester: this.telegramRequester(route, message),
        onRollback: () => this.clearActivityIndicator(route),
      })) return;
...
export async function sendSessionNotification(
  ...
  const imageHint = status === "completed" && !route.notification.structuredAnswer && route.notification.latestImages?.count
    ...
    : "";
  await runtime.sendToBoundChat(route.sessionKey, `${fallback}${imageHint}`, { terminalStatus: status });
}
