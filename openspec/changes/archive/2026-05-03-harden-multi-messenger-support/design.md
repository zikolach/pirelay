...
13. **Make Discord connect QR-driven, PIN-based, and locally confirmed.**
   - Decision: Discord setup treats Application ID/clientId as required for the polished onboarding path because it is needed to build the Discord OAuth2 bot invite/open URL. `/relay connect discord` SHALL render a QR code for that Discord URL when clientId is configured, show shared-server/DM guidance, and display a short human-entered pairing PIN such as `482-913` for the Discord DM command (`relay pair <pin>` as the reliable form, `/start <pin>` as a compatibility alias). Short-code pairing SHALL require local Pi approval unless the messenger identity is already allow-listed in config or trusted in local PiRelay state. The local approval UI SHALL offer at least allow-once, trust-this-user, and deny choices; trust-this-user records the messenger identity in local state rather than mutating config automatically.
   - Rationale: Discord does not provide a Telegram-style `t.me/<bot>?start=<payload>` deep link that both opens a bot DM and delivers arbitrary pairing state. A QR invite/open link plus a short PIN gives a similar guided experience while preserving safety through explicit local approval. Keeping trust state separate from config avoids surprising edits to env-managed or read-only configuration while still letting repeat users skip confirmation.
   - Alternatives: keep long cryptographic nonces for Discord. Rejected because they are good for deep links but poor for manual Discord DM entry. Mutate `allowUserIds` directly from confirmation. Rejected as a default because config may be managed externally; admin-managed allow-lists remain supported. Implement full OAuth callback pairing now. Deferred because local-only OAuth works only on the same machine and QR/mobile OAuth needs a rendezvous service or manual fallback.
