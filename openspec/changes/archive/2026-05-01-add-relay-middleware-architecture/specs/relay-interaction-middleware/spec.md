...
The system SHALL provide equivalent middleware behavior in both in-process and broker runtimes.

#### Scenario: Broker handles inbound event
- **WHEN** the singleton broker owns channel polling and receives an authorized inbound event
- **THEN** the broker and session-owning client exchange normalized pipeline data so middleware behavior matches in-process runtime behavior

#### Scenario: Middleware action targets offline session
- **WHEN** middleware resolves an action that requires an online session but the selected session is offline
- **THEN** the system reports the offline state to the channel and does not silently drop the action

#### Scenario: Stale middleware action is invoked
- **WHEN** a channel callback or text command references middleware state that is expired or no longer current
