...
- Add a Slack slash-command surface centered on `/relay`, including Socket Mode/webhook payload routing and setup guidance.
- Extend the generated Slack app manifest to include the `/relay` command and required interactivity/event settings without secrets.
...
- `relay-setup-tui`: Slack setup manifest and wizard guidance must include the `/relay` slash-command surface and explain native-command caveats.
