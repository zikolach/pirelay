...
- **AND** the setup guidance explains that Slack must deliver `/pirelay` through the installed app manifest before native slash invocations work, while plain `pirelay <command>` text remains the reliable fallback when slash setup is unavailable
