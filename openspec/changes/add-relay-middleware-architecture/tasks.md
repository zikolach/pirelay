...
- [x] 3.1 Add characterization tests for existing Telegram command, prompt, guided answer, full output, image, pause/resume, and callback behavior.
- [x] 3.2 Move Telegram command/action parsing behind middleware-compatible normalized events.
- [x] 3.3 Move existing media validation/download and latest-output formatting into built-in middleware modules or adapters without changing user-visible behavior.
- [x] 3.4 Preserve `/telegram-tunnel` compatibility, current config paths, and existing binding metadata.

## 4. Runtime and broker integration

- [x] 4.1 Update in-process runtime to invoke the middleware pipeline for inbound and outbound interactions.
- [x] 4.2 Update broker runtime and broker process to exchange normalized pipeline events/results over IPC.
- [x] 4.3 Add IPC versioning or compatibility checks for middleware envelope evolution.
- [x] 4.4 Ensure stale, offline, paused, unauthorized, and revoked states are handled consistently across runtimes.

## 5. Accessibility readiness

- [x] 5.1 Add extension points required by future audio accessibility middleware: audio media events, transcript prompt results, spoken-output requests, repeat/read-last actions, and confirmation-needed actions.
...
- [x] 6.4 Run typecheck and the full test suite.
