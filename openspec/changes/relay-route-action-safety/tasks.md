...
- [x] 2.4 Add focused unit tests for typed outcomes, unavailable detection, and non-unavailable error preservation.
...
- [x] 3.4 Add tests proving stale idle, model, or workspace probes render routes offline/unavailable rather than online idle, online busy, or online with missing model only.

## 4. Prompt Operation Safety

- [x] 4.1 Implement a shared prompt operation helper that probes availability, chooses busy delivery mode, reserves requester context, invokes `sendUserMessage`, and returns typed accepted/unavailable/failure outcomes.
...
- [x] 4.6 Migrate broker `deliverPrompt` handling to the shared prompt operation or a thin equivalent wrapper.
- [x] 4.7 Add cross-adapter tests proving unavailable prompt races clear requester state, stop activity/typing/reaction refreshes, roll back shared-room destinations, and do not mark messenger runtime health unhealthy.
...
- [x] 5.4 Migrate Discord abort and compact command paths to shared control helpers.
- [x] 5.5 Migrate Slack abort and compact command paths to shared control helpers.
- [x] 5.6 Migrate broker abort and compact actions to shared control helpers or a thin equivalent wrapper.
- [x] 5.7 Add tests proving abort unavailable races roll back `abortRequested`, compact unavailable races return safe unavailable responses, and successful/idle control behavior remains unchanged.
