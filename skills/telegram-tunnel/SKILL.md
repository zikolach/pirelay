...
If multiple Pi sessions are paired to the same Telegram chat, use `/sessions` to list numbered sessions with labels, online/offline state, active marker, and idle/busy state. Use `/use <number|label>` to switch the active one, or `/to <session> <prompt>` to send a one-shot prompt without changing the active session. Pair with `/telegram-tunnel connect docs` or another short label when you want friendlier names; otherwise PiRelay falls back to the Pi session name, project folder name, session file basename, then a short session id.
